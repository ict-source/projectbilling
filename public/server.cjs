...
//     }
//   });
//   db.run(`ALTER TABLE patients ADD COLUMN firebase_uid TEXT`, (err) => {
//     if (err && !err.message.includes('duplicate column name')) {
//       console.error('Error adding firebase_uid column:', err);
//     }
//   });
...
//   // Insert sample data after a short delay to ensure tables are created
//   setTimeout(() => {
...
app.post('/api/google-login', (req, res) => {
  ...
  console.log('Google login attempt:', { email, name, firebaseUid });
  ...
  db.get("SELECT * FROM patients WHERE email = ?", [email], (err, row) => {
    ...
    if (row) {
      ...
      if (!row.firebase_uid) {
        db.run("UPDATE patients SET firebase_uid = ? WHERE id = ?", [firebaseUid, row.id], (updateErr) => {
          if (updateErr) {
            console.error('Update error:', updateErr);
          }
        });
      }
      console.log('Existing user:', { id: row.id, name: row.name, email: row.email, role: row.role });
      res.json({ success: true, user: { id: row.id, name: row.name, email: row.email, role: row.role, patient_id: row.patient_id } });
    } else {
      // User doesn't exist, create new user
      const firstName = name.split(' ')[0] || '';
      const lastName = name.split(' ').slice(1).join(' ') || '';
      const patientId = `GOOGLE_${Date.now()}`; // Generate a temporary patient ID

      db.run("INSERT INTO patients (email, name, first_name, last_name, patient_id, role, email_verified, firebase_uid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        [email, name, firstName, lastName, patientId, 'patient', 1, firebaseUid], function(insertErr) {
        if (insertErr) {
          console.error('Insert error:', insertErr);
          return res.status(500).json({ success: false, message: 'Failed to create user' });
        }
        const newUserId = this.lastID;
        console.log('New Google user created:', { id: newUserId, name, email });
        res.json({ success: true, user: { id: newUserId, name, email, role: 'patient', patient_id: patientId } });
      });
    }
  });
});
...
app.post('/api/register', (req, res) => {
  ...
  db.run("INSERT INTO patients (email, password, first_name, last_name, name, phone, patient_id, role, email_verified, verification_token) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <a href="https://projectbill-c4b5e.web.app/verify-email?token=${verificationToken}" style="background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px; display: inline-block;">Verify Email</a>
          <p>If the button doesn't work, copy and paste this link into your browser:</p>
          <p>https://projectbill-c4b5e.web.app/verify-email?token=${verificationToken}</p>
    ...
    if (isEmailConfigured) {
      ...
      console.log('Development mode: Email not sent. Verification link:', `https://projectbill-c4b5e.web.app/verify-email?token=${verificationToken}`);
  ...
  db.get("SELECT * FROM patients WHERE email = ? AND email_verified = 0", [email], (err, row) => {
    ...
    db.run("UPDATE patients SET verification_token = ? WHERE email = ?", [verificationToken, email], (updateErr) => {
      ...
      if (isEmailConfigured) {
        ...
        console.log('Development mode: Resend email not sent. Verification link:', `https://projectbill-c4b5e.web.app/verify-email?token=${verificationToken}`);
        console.log('Email content:', mailOptions.html);
...
app.post('/api/log-download', async (req, res) => {
  const { userId, userName, patientId, action, details } = req.body;
  ...
  try {
    let spreadsheetId = process.env.GOOGLE_ACTIVITY_SHEET_ID || process.env.GOOGLE_SHEET_ID;
    let title = 'Activity Log';

    if (!spreadsheetId || spreadsheetId === 'your-spreadsheet-id') {
      // Create a new spreadsheet for activity logs
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const spreadsheet = await sheets.spreadsheets.create({
        resource: {
          properties: {
            title: `ActivityLog-${timestamp}`,
          },
        },
      });
      spreadsheetId = spreadsheet.data.spreadsheetId;
      ...
      await sheets.spreadsheets.values.update({
        ...
        range: 'Activity!A1',
        valueInputOption: 'RAW',
        resource: { values: [['Timestamp', 'User ID', 'User Name', 'Patient ID', 'Action', 'Details']] },
      });
    }

    // Append the log entry
    const timestamp = new Date().toISOString();
    const values = [[timestamp, userId, userName, patientId, action, details]];
