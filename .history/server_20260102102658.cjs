...
function initializeDatabase() {
  ...
  db.get("SELECT COUNT(*) as count FROM patients", (err, row) => {
    if (row.count === 0) {
      db.run("INSERT INTO patients (email, password, name, phone, patient_id, status, admission_date, room, total_billed, pending_balance, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", ['john.doe@email.com', 'password123', 'John Doe', '(555) 123-4567', 'PAT-001', 'admitted', '2024-12-15', '203A', 2500.00, 1245.00, 'patient']);
      db.run("INSERT INTO patients (email, password, name, phone, patient_id, status, admission_date, room, total_billed, pending_balance, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", ['jane.smith@email.com', 'password123', 'Jane Smith', '(555) 987-6543', 'PAT-002', 'admitted', '2024-12-18', '105B', 1200.00, 850.00, 'patient']);
      db.run("INSERT INTO patients (email, password, name, phone, patient_id, status, admission_date, room, total_billed, pending_balance, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", ['robert.j@email.com', 'password123', 'Robert Johnson', '(555) 456-7890', 'PAT-003', 'discharged', '2024-12-10', '-', 4500.00, 0, 'patient']);
