...
function initializeDatabase() {
  ...
  db.get("SELECT COUNT(*) as count FROM patients", (err, row) => {
    if (row.count === 0) {
      db.run("INSERT INTO patients (email, password, name) VALUES (?, ?, ?)", ['patient@example.com', 'password123', 'John Doe']);
...
app.post('/api/login', (req, res) => {
  ...
  db.get("SELECT * FROM patients WHERE email = ? AND password = ?", [email, password], (err, row) => {
    ...
    if (row) {
      res.json({ success: true, user: { id: row.id, name: row.name, email: row.email } });
    } else {
...
app.post('/api/register', (req, res) => {
  ...
  db.run("INSERT INTO patients (email, password, first_name, last_name, name, phone, patient_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
    [email, password, firstName, lastName, name, phone, patientId], function(err) {
    if (err) {
