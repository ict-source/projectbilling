...
function initializeDatabase() {
  ...
  db.get("SELECT COUNT(*) as count FROM patients", (err, row) => {
    if (row.count === 0) {
      db.run("INSERT INTO patients (email, password, name, phone, patient_id, role) VALUES (?, ?, ?, ?, ?, ?)", ['john.doe@email.com', 'password123', 'John Doe', '(555) 123-4567', 'PAT-001', 'patient']);
      db.run("INSERT INTO patients (email, password, name, phone, patient_id, role) VALUES (?, ?, ?, ?, ?, ?)", ['jane.smith@email.com', 'password123', 'Jane Smith', '(555) 987-6543', 'PAT-002', 'patient']);
      db.run("INSERT INTO patients (email, password, name, phone, patient_id, role) VALUES (?, ?, ?, ?, ?, ?)", ['robert.j@email.com', 'password123', 'Robert Johnson', '(555) 456-7890', 'PAT-003', 'patient']);
